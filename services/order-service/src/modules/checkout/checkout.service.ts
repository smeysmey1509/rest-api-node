...
import { invalidateCart, setCachedCart } from "@shared/infrastructure/redis/cache";
...
export const checkoutService = {
  async checkout(userId: string, body: CheckoutBody) {
    ...
    await publishDomainEvent(DomainEventNames.OrderCreated, {
      orderId: String(order._id),
      userId,
      paymentId: String(payment._id),
      total,
    }).catch((error) => console.error("Failed to publish order.created event:", error));
