...
export const paymentService = {
  ...
  async markSuccess(payment: IPayment, raw: unknown, paidAt: Date) {
    ...
    try {
      ...
      await session.commitTransaction();
      await publishDomainEvent(DomainEventNames.PaymentCompleted, {
        paymentId: String(freshPayment._id),
        orderId: String(order._id),
        userId: String(order.user),
        amount: freshPayment.amount,
      }).catch((error) => console.error("Failed to publish payment.completed event:", error));
