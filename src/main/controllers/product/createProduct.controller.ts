 ...
 * Accepts either:
 *  - [{ sku, price, stock, attributes, images, isActive }]
 *  - Or advanced: { inventory: { onHand, reserved, safetyStock } }
 * If only stock provided, convert to inventory.onHand.
 */
type InputVariant = {
  sku: string;
  price: number | string;
...
function normalizeSeo(raw: unknown): { title?: string; description?: string; keywords?: string[] } | undefined {
  const seo = parseJSON<any>(raw, undefined as any);
  if (!seo) return undefined;
  const keywords =
    Array.isArray(seo.keywords) ? seo.keywords.map((k: any) => String(k)) :
    typeof seo.keywords === "string" && seo.keywords.trim() ? seo.keywords.split(",").map((k: string) => k.trim()) :
    [];
  return {
